// Ported (Phase 1A / Task 5) from assets/js/sim/engine.js.
//
// FAITHFUL 1:1 PORT. The only changes from the legacy source are:
//   (a) IIFE + global-attach wiring -> ES module imports/exports,
//   (b) the service-locator reads (window/globalThis.SimRouter,
//       .MISSIONS_CONFIG, .DOCK_RANGE) replaced with explicit imports, and
//   (c) added type annotations (params, returns, entity literals, generics).
// No logic, constant, string-literal, event-copy, or control-flow changes.
//
// Type-only accommodations (no runtime effect): a handful of precise casts
// (`as LonLat` on `.slice()`, `as MissionType[]` on `Object.keys`, `as
// LineString` on GeoJSON geometry) and non-null assertions guarded by the
// legacy code's own invariants. See task-5-report.md for the full rationale.
...
// Bound once so the module-scope helpers below can reference the router the
// same way the legacy per-function `const R = (window||globalThis).SimRouter`
// reads did.
const R = SimRouter

// ---- public factory options / specs ----
...
export interface Engine {
  ...
  requests: Map<string, FlightRequest>
  tracks: Map<string, Track>
  events: SimEvent[]
  roads: FeatureCollection
  rand: () => number
  airborneTarget: number
  _schedAcc: number
  _ambientAcc: number
  _ambientIdx: number
  _missionSeq: number
  ...
  approveRequest(id: string): Mission
  declineRequest(id: string): boolean
  taskTrack(id: string): Mission
  dismissTrack(id: string): boolean
  commandRTB(id: string): boolean
  commandHold(id: string, on: boolean): boolean
  setManual(id: string, on: boolean): boolean
  manualGoto(id: string, lonlat: LonLat): boolean
  manualQueue(id: string, lonlat: LonLat): boolean
  nudgeAlt(id: string, delta: number): boolean
}
...
const DETECTION_MSGS: Record<MissionType, (id: string) => string> = {
...
const REQUEST_CUSTOMERS: Record<MissionType, [string, string][]> = {
  security: [
  ...
  parks: [
    ['DMT', 'DEPT OF MUNICIPALITIES & TRANSPORT'],
...
// ---------- helpers ----------
function isFiniteXY(p: unknown): p is LonLat {
  return Array.isArray(p) && Number.isFinite(p[0]) && Number.isFinite(p[1])
}

function clampPos(p: LonLat): LonLat {
  return [Math.min(LON_MAX, Math.max(LON_MIN, p[0])), Math.min(LAT_MAX, Math.max(LAT_MIN, p[1]))]
}
...
function randomOffsetPoint(rnd: () => number, center: LonLat, maxM: number, minM?: number): LonLat {
  const ang = rnd() * Math.PI * 2
  const dist = (minM || 0) + rnd() * (maxM - (minM || 0))
  return R.offsetMeters(center, Math.cos(ang) * dist, Math.sin(ang) * dist)
}

function nearestRoad(roads: FeatureCollection, coords: LonLat): Feature | null {
  let best: Feature | null = null,
    bestD = Infinity
  const features = (roads && roads.features) || []
  for (const f of features) {
    const line = f && f.geometry && (f.geometry as LineString).coordinates
...
function dockRangeM(dock: Dock): number {
  const DR = DOCK_RANGE
...
function pullToRange(pt: LonLat, center: LonLat, rangeM: number): LonLat {
...
function clampToRange(route: LonLat[], center: LonLat, rangeM: number): LonLat[]
function clampToRange(route: LonLat[] | null, center: LonLat, rangeM: number): LonLat[] | null
function clampToRange(route: LonLat[] | null, center: LonLat, rangeM: number): LonLat[] | null {
...
function corridorNearDock(
  R: typeof SimRouter,
  road: Feature,
  dockCoords: LonLat,
  lengthKm: number,
): LonLat[] | null {
  const line = (road.geometry as LineString).coordinates as LonLat[]
  if (!Array.isArray(line) || line.length < 2) return null
  let total = 0
  const cum = [0]
  for (let i = 1; i < line.length; i++) {
    total += R.distM(line[i - 1], line[i]) / 1000
    cum.push(total)
  }
  let bi = 0,
    bd = Infinity
...
// ---------- engine factory ----------
function create(opts?: CreateOpts): Engine {
  opts = opts || {}
  const R = SimRouter
  const docksInput = opts.docks || []
  const roads: FeatureCollection = opts.roads || { type: 'FeatureCollection', features: [] }

  const engine = {
    now: opts.now || 0,
    docks: new Map<string, Dock>(),
    drones: new Map<string, Drone>(),
    missions: new Map<string, Mission>(),
    requests: new Map<string, FlightRequest>(),
    ...
    _subscribers: [] as ((ev: SimEvent) => void)[],
  } as Engine

  // ---- boot docks + drones ----
  const allDocksList: Dock[] = []
  for (const src of docksInput) {
    ...
    const drone: Drone = {
      ...
      dockId: src.id,
      pos: src.coords.slice() as LonLat,
      alt: 0,
      heading: 0,
      speedMs: 0,
      battery: 100,
    ...
    const dock: Dock = {
      ...
      emirate: src.emirate || '',
      coords: src.coords.slice() as LonLat,
  ...
  function emit(
    ...
    message: string,
    code?: string,
    extra?: { dockId?: string; requestId?: string; trackId?: string },
  ): void {
    const ev: SimEvent = { time: engine.now, level: level, source: source, message: message }
    ...
    for (const cb of engine._subscribers) {
      try {
        ...
        /* subscriber errors must not break the sim */
      }
    }
  }
  ...
  // ---- mission lifecycle ----
  function finalizeMission(mission: Mission, drone: Drone): void {
    ...
    if (mission.requestId) {
      ...
      if (req && req.status === 'approved') {
        ...
        emit(
          'info',
          'OPS',
          'REQUEST ' + req.id! + ' FULFILLED · ' + req.customer,
          'REQUEST_FULFILLED',
          {
            requestId: req.id!,
          },
    ...
    if (mission.trackId) {
      ...
      if (track && track.status === 'tasked') {
        ...
        emit('info', 'OPS', 'TRACK ' + track.id + ' RESOLVED · ' + track.label, 'TRACK_RESOLVED', {
          trackId: track.id,
        })
      }
    }
  ...
  function pruneFinishedMissions(): void {
  ...
  function advanceLeg(drone: Drone, dt: number): void {
  ...
  function checkMilestones(mission: Mission, drone: Drone): void {
  ...
  function emitDetection(mission: Mission, drone: Drone): void {
    const fn = DETECTION_MSGS[mission.type]
    if (!fn) return
    let track: Track | null = null
    if (
      TRACK_LABELS[mission.type] &&
      engine.rand() < TRACK_SPAWN_P &&
  ...
  function updateDrone(drone: Drone, dt: number): void {
    ...
    switch (drone.state) {
      case 'takeoff': {
        ...
        if (drone._timer <= 0 && mission) {
          ...
          drone._leg = [dock.coords.slice() as LonLat, mission.waypoints[0]]
      ...
      case 'on-task': {
        if (!mission) {
          beginRtb(drone, dock, null, false)
          break
        }
        advanceLeg(drone, dt)
      ...
      case 'rtb': {
        ...
        if (drone._legProgress >= 1) {
          ...
          drone.pos = dock.coords.slice() as LonLat
          dock.state = 'landing'
  ...
  engine.createMission = function (spec) {
    ...
    if (!Number.isFinite(params.speedMs) || params.speedMs <= 0)
      params.speedMs = config.defaults.speedMs
  ...
  engine.launchPreset = function (type, opts) {
    ...
    if (opts.dockId) {
      ...
      for (const dock of engine.docks.values()) {
        if (
          dock.state === 'ready' &&
          dock.battery >= SCHED_MIN_BATTERY &&
          dock.drone &&
          dock.drone.state === 'docked' &&
          !reserved.has(dock.id)
        ) {
    ...
    if (opts.near && isFiniteXY(opts.near)) {
      candidates = candidates
        .slice()
        .sort((a, b) => R.distM(a.coords, opts.near!) - R.distM(b.coords, opts.near!))
    }
    ...
    throw (lastErr as Error) || new Error('Could not generate a route for ' + type)
  }
  ...
  function planRequestRoute(request: FlightRequest, dock: Dock): LonLat[] | null {
    ...
    switch (config.pattern) {
      ...
      case 'atob': {
        route = R.atob(dock.coords.slice() as LonLat, request.coords.slice() as LonLat)
        break
      }
      default:
        return null
  ...
  function reservedDockIds(): Set<string> {
  ...
  function runRequestPass(): void {
    ...
    const type = pick(engine.rand, Object.keys(MISSIONS_CONFIG) as MissionType[])
    ...
    const priority: Priority =
      type === 'emergency' ? 'URGENT' : engine.rand() < 0.3 ? 'PRIORITY' : 'ROUTINE'
    const altM = Math.round(
      Math.min(120, Math.max(40, config.defaults.altM + (engine.rand() * 2 - 1) * 20)),
    )
    const speedMs =
      Math.round(
        Math.min(21, Math.max(5, config.defaults.speedMs + (engine.rand() * 2 - 1) * 2)) * 10,
      ) / 10
  ...
  function dockEligible(dock: Dock | null | undefined): dock is Dock {
    return !!(
      dock &&
      dock.state === 'ready' &&
      dock.battery >= SCHED_MIN_BATTERY &&
  ...
  engine.approveRequest = function (id) {
    ...
    if (!dockEligible(dock)) {
      ...
      for (const d of engine.docks.values()) {
        ...
        if (dist <= dockRangeM(d) * 0.9 && dist < bestD) {
          bestD = dist
          best = d
        }
      }
    ...
    emit(
      ...
      'REQUEST_APPROVED',
      { requestId: request.id!, dockId: dock.id },
    )
    return mission
  }
  ...
  engine.declineRequest = function (id) {
    ...
    emit(
      'info',
      'OPS',
      'REQUEST ' + request.id! + ' DECLINED · ' + request.customer,
      'REQUEST_DECLINED',
      {
        requestId: request.id!,
      },
  ...
  function tickTracks(): void {
    for (const t of engine.tracks.values()) {
      if (t.status === 'active' && engine.now >= t.expiresAt) {
        t.status = 'expired'
        emit('info', 'OPS', 'TRACK ' + t.id + ' EXPIRED · NO ACTION TAKEN', 'TRACK_EXPIRED', {
          trackId: t.id,
        })
      }
    }
  ...
  engine.taskTrack = function (id) {
    ...
    emit(
      ...
      'TRACK_TASKED',
      { trackId: track.id, dockId: best.id },
    )
    return mission
  }
  ...
  function divertToTrack(drone: Drone, track: Track): Mission {
    const dock = engine.docks.get(drone.dockId)!
    ...
    drone._holdUntil = 0
    drone._preHoldState = null
    drone._leg = [drone.pos.slice() as LonLat, waypoints[0]]
    drone._legDistKm = R.pathLengthKm(drone._leg) || 0.0001
    drone._legProgress = 0
    ...
    emit(
      ...
      'TRACK_TASKED',
      { trackId: track.id, dockId: dock.id },
    )
    return mission
  }
  ...
  engine.dismissTrack = function (id) {
    ...
    track.status = 'resolved'
    emit('info', 'OPS', 'TRACK ' + track.id + ' DISMISSED · OPERATOR', 'TRACK_DISMISSED', {
      trackId: track.id,
    })
    return true
  }
  ...
  engine.commandRTB = function (id) {
    ...
    if (drone.state !== 'transit' && drone.state !== 'on-task' && drone.state !== 'hold')
      return false
    const dock = engine.docks.get(drone.dockId)
  ...
  engine.setManual = function (id, on) {
    ...
    if (on) {
      ...
      drone._holdUntil = 0
      drone._manualQueue = []
      drone._manualSpeed = Math.min(
        18,
        (mission && mission.params && mission.params.speedMs) || drone.speedMs || 12,
      )
      drone.state = 'manual'
      drone.speedMs = 0
  ...
  function manualTarget(drone: Drone, lonlat: LonLat): LonLat {
  ...
  function ambientNetwork(): string {
    return 'NETWORK · E& 5G 99.98% · GCAA SYNC OK'
  }
  function ambientDock(): string {
  ...
  function tickFaults(dt: number): void {
  ...
  engine.tick = function (dt) {
    ...
    while (engine._requestAcc >= engine._requestNextS) {
      ...
      engine._requestNextS =
        REQUEST_MIN_INTERVAL_S + engine.rand() * (REQUEST_MAX_INTERVAL_S - REQUEST_MIN_INTERVAL_S)
      runRequestPass()
    }
...
export const SimEngine = { create, mulberry32 }
